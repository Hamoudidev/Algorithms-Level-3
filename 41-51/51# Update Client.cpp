...
void UpdateAccount(vector <stClients>& vClients, string TargetAccountNumber) {
	stClients Client;
	...
	if (FindAccount(vClients, TargetAccountNumber, Client)) {
		...
		if (tolower(ConfirmDelete) == 'y') {
			cout << "\n";
			stClients UpdatedData = ReadUpdates();

			for (stClients& C : vClients) {
				if (C.accNumber == TargetAccountNumber) {
					UpdatedData.accNumber = C.accNumber;
					C = UpdatedData;
					break;
				}
			}
...
int main() {
	...
	UpdateAccount(ClientsData, accNum);
